...
front_server_url = ['http://example.com/receivejson']
model_server_url = ['http://example.com/receivejson']
...
def read_chapture():
    try:
        ...
        if data is not None:
            ...
            for n in range(3):
                data2model[f'image_{n}'] = capture(n, 640,480)
                
            # >model
            response = requests.post(model_server_url[0], json = data2model)
            response_data = response.json()
